//
//  BW64_for_libsnfile1.c
//  BW64_for_libsndfile
//
//  Created by Jack Reynolds on 12/01/2018.
//  Copyright © 2018 Jack Reynolds. All rights reserved.
//
/*
...
#define RIFF_DOWNGRADE_BYTES	((sf_count_t) 0xffffffff) // use riffSizeHigh and riffSizeLow in ds24 instead
...
{	WAVLIKE_PRIVATE	*wpriv ;
	...
	if (marker != BW64_MARKER || marks[1] != WAVE_MARKER)
...
//should there be a dummy parameter = 0 here?
	...
	*/
			...
			case axml_MARKER :
				if ((error = wavlike_read_axml_chunk (psf, chunk_size)) != 0)
					return error ;
			...
			case data_MARKER :
				...
				{	if (chunk_size == 0xffffffff)
					...
					{	psf_log_printf (psf, "%M : 0x%x\n**** Weird, BW64 file without a 'ds64' chunk and no valid 'data' size.\n", marker, chunk_size) ;
		...
		/* The 'data' chunk, a chunk size of 0xffffffff (equal to -1) means that the 'data' chunk size
	...
	switch (format)
	{
		...
		case WAVE_FORMAT_PCM :
			psf->sf.format = SF_FORMAT_BW64 | u_bitwidth_to_subformat (psf->bytewidth * 8) ;
			break ;

		case WAVE_FORMAT_MULAW :
		case IBM_FORMAT_MULAW :
			psf->sf.format = (SF_FORMAT_BW64 | SF_FORMAT_ULAW) ;
			break ;

		case WAVE_FORMAT_ALAW :
		case IBM_FORMAT_ALAW :
			psf->sf.format = (SF_FORMAT_BW64 | SF_FORMAT_ALAW) ;
			break ;

		case WAVE_FORMAT_MS_ADPCM :
			psf->sf.format = (SF_FORMAT_BW64 | SF_FORMAT_MS_ADPCM) ;
			*blockalign = wav_fmt->msadpcm.blockalign ;
			*framesperblock = wav_fmt->msadpcm.samplesperblock ;
			break ;

		case WAVE_FORMAT_IMA_ADPCM :
			psf->sf.format = (SF_FORMAT_BW64 | SF_FORMAT_IMA_ADPCM) ;
			*blockalign = wav_fmt->ima.blockalign ;
			*framesperblock = wav_fmt->ima.samplesperblock ;
			break ;

		case WAVE_FORMAT_GSM610 :
			psf->sf.format = (SF_FORMAT_BW64 | SF_FORMAT_GSM610) ;
			break ;

		case WAVE_FORMAT_IEEE_FLOAT :
			psf->sf.format = SF_FORMAT_BW64 ;
			psf->sf.format |= (psf->bytewidth == 8) ? SF_FORMAT_DOUBLE : SF_FORMAT_FLOAT ;
			break ;

		case WAVE_FORMAT_G721_ADPCM :
			psf->sf.format = SF_FORMAT_BW64 | SF_FORMAT_G721_32 ;
			break ;
...
{	WAVLIKE_PRIVATE	*wpriv ;
	...
	{   case SF_FORMAT_PCM_U8 :
		...
		default:
			/* fmt : format, channels, samplerate */
			psf_binheader_writef (psf, "4224", BHW4 (fmt_size), BHW2 (WAVE_FORMAT_UNKNOWN), BHW2 (psf->sf.channels), BHW4 (psf->sf.samplerate)) ;
			break;
	} ;
...
{	WAVLIKE_PRIVATE	*wpriv ;
	...
	switch (command)
	{
			case SFC_BW64_AUTO_DOWNGRADE :
			if (psf->have_written == 0)
				wpriv->bw64_downgrade = datasize ? SF_TRUE : SF_FALSE ;
			return wpriv->bw64_downgrade ;
