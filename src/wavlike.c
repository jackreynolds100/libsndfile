...
#define WAV_CHNA_MIN_CHUNK_SIZE   4
#define WAV_CHNA_TRACK_CHUNK_SIZE 40
...
{	SF_CHNA_INFO_FIXED *b ;
	...
	/* calculate the chna chunk size based on number of UIDs */
	chna_chunk_size = WAV_CHNA_MIN_CHUNK_SIZE + (b->numUIDs * WAV_CHNA_TRACK_CHUNK_SIZE);

	/* header and chunk size */
  	psf_binheader_writef (psf, "m4", BHWm (chna_MARKER), BHW4 (chna_chunk_size)) ;
...
{	char *b ;
	...
	if (chunksize >= WAV_AXML_MAX_CHUNK_SIZE)
	{	psf_log_printf (psf, "axml : %u too big to be handled\n", chunksize) ;
		psf_binheader_readf (psf, "j", chunksize) ;
		return 0 ;
		} ;

	if ((psf->axml_var = axml_var_alloc (chunksize)) == NULL)
	{	psf->error = SFE_MALLOC_FAILED ;
		return psf->error ;
		} ;
...
{	char *b ;
	...
	/* header and chunk size */
  	psf_binheader_writef (psf, "m4", BHWm (axml_MARKER), BHW4 (psf->axml_len)) ;

	/* axml data - Note: there must be an even number of bytes */
	psf_binheader_writef (psf, "b", BHWv (b), BHWz (psf->axml_len)) ;
