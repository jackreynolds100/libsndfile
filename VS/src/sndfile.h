...
{	/* Major formats. */
  ...
  SF_FORMAT_BW64			= 0x230000,   /* BW64 WAV file */
...
{	SFC_GET_LIB_VERSION				= 0x1000,
	...
	SFC_GET_CHNA_INFO						= 0x10F4,
	SFC_SET_CHNA_INFO						= 0x10F5,

	SFC_GET_AXML_INFO						= 0x10FA,
	SFC_SET_AXML_INFO						= 0x10FB,
	...
	/*
	** RF64 and BW64 files can be set so that on-close, writable files that have less
	** than 4GB of data in them are converted to RIFF/WAV, as per EBU
	** recommendations.
	*/
	SFC_RF64_AUTO_DOWNGRADE			= 0x1210,
	SFC_BW64_AUTO_DOWNGRADE			= 0x1211,
...
/*below entries taken from BS.2088*/
...
#define SF_CHNA_INFO_VAR(audioID_size) \
			struct \
			{ char   	ckID	[4] ; \
				uint32_t 		ckSize ; \
				uint16_t 		numTracks ; \
				uint16_t 		numUIDs ; \
				SF_CHNA_TRACK 		audioID	[audioID_size] ; \
			}
typedef SF_CHNA_INFO_VAR (256) SF_CHNA_INFO ;


#define SF_AXML_INFO_VAR(xmlData_size) \
	struct \
 			{ char   	ckID	[4] ; \
			  uint32_t ckSize ; \
				char 		xmlData	[xmlData_size] ; \
			}

typedef SF_AXML_INFO_VAR (256) SF_AXML_INFO ;
